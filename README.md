...
* `.matches` _Object_ the tournament's matches (for by-round access, go via the `[Round](#object-Round)` objects in `.rounds`)
...
Represents an individual player. Note that matches are played between _entrants_ consisting of one or more players, not players themselves.
...
* `.pointsFor` _Object or undefined_ the points received by any given entrant for this swiss match, `undefined` for playoff matches
